...
def _subject_detail_view(subject):
    ...
    if not learners_with_empty.empty:
        st.warning(
            ...
            + ", ".join(
                f"{row['Stagiaire']} ({int(row['Réponses vides'])})"
...
def _build_pie_data_for_learner(subjects, learner_name: str):
    ...
    return pd.DataFrame(
        {"Type": ["Bonnes réponses", "Mauvaises réponses"], "Valeur": [correct, incorrect]}
    )
...
def _synthesis_view(subjects, synthesis_df):
    ...
    with col1:
        ...
        fig_learner = px.pie(
            ...
            values="Valeur",
            names="Type",
            title=f"Répartition des réponses - {selected_learner}",
            hole=0.35,
        )
    ...
    with col2:
        ...
        if not theme_df.empty:
            ...
            val = float(
                theme_df.loc[theme_df["Thématique"] == selected_theme, "Bonnes réponses moyennes"].iloc[0]
            )
            ...
            fig_theme = px.pie(
                ...
                values="Valeur",
                names="Type",
                title=f"Répartition moyenne groupe - {selected_theme}",
                hole=0.35,
            )
    ...
    fig_group = px.bar(
        ...
        x="Stagiaire",
        y="Moyenne générale",
        title="Moyenne générale par stagiaire (groupe)",
    )
...
def _manual_mode():
    ...
    for subject_name in selected_subjects:
        ...
        edited = st.data_editor(base, num_rows="fixed", key=f"editor_{subject_name}", use_container_width=True)
        subject_tables[subject_name] = edited

    if st.button("Calculer les résultats"):
        subjects = build_subjects_from_manual_tables(subject_tables, learners)
        subjects = score_all_subjects(subjects)
        synthesis_df = build_synthesis(subjects)
        ...
        st.download_button(
            ...
            data=export_manual_workbook(section_name, subjects, synthesis_df),
            file_name=f"resultats_{section_name.replace(' ', '_')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
        st.download_button(
            "Télécharger synthèse CSV",
            data=export_synthesis_csv(synthesis_df),
            file_name=f"synthese_{section_name.replace(' ', '_')}.csv",
            mime="text/csv",
        )
